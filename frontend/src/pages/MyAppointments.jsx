  ...
  const initPay = (order) => {
    const options = {
      ...
      handler: async (response) => {
        ...
        try {
          const {data} = await axios.post(backendUrl + '/api/user/verify-razoroay',response,{headers:{token}})
          if(data.success){
            getUserAppointment()
            ...
            <div className="flex flex-col gap-2 justify-end">
              ...
              {!item.cancelled && !item.payment && !item.isCompleted && (
                <button
                  ...
                  className="text-sm text-stone-600 text-center sm:min-w-48 py-2 border rounded hover:bg-primary hover:text-white transition-all duration-300"
                >
                  Pay Online
                </button>
              )}
              {!item.cancelled && !item.isCompleted && (
                <button
                  ...
                  className="text-sm text-stone-600 text-center sm:min-w-48 py-2 border rounded hover:bg-red-600 hover:text-white transition-all duration-300"
                >
                  Cancel appointment
                </button>
              )}
              {item.cancelled && !item.isCompleted && (
                <button className="sm:min-w-48 py-2 border text-red-500">
