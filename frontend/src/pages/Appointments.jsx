  ...
  const getAvailableSlots = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      while (currentDate < endTime) {
        ...
        if (docInfo) {
          const isSlotAvailbale = docInfo?.slots_booked?.[slotDate]?.includes(
            slotTime
          )
            ? false
            : true;
      ...
      <div>
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
            ...
            {docSlots.length &&
              docSlots[slotIndex].map((item, index) => (
                <p
                  ...
                  className={`text-sm font-light flex-shrink-0 px-3 py-1 rounded-full cursor-pointer ${
                    item.time === slotTime
                      ? "bg-primary text-white"
                      : "text-gray-600 border border-grat-300"
                  }`}
                  key={index}
                >
          ...
          <button
            ...
            className="text-white bg-primary text-sm font-light px-10 py-3 rounded-lg mt-7"
          >
            Book an appointment
          </button>
        </div>
